...
/* The size of time_t is fungible in SR0660 and above. */
int   __gl_time_t_is_32_bits             = 0;
#else
/* Use the default size for time_t so that binder -U doesn't
   have to be passed on every 32bit target. */
int   __gl_time_t_is_32_bits             = (sizeof (time_t) == 4);
#endif
