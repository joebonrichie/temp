...
/* Return true if LOCATION is a useful location in function FUN.  */

static inline bool
useful_location_p (struct function *fun, location_t location)
{
  const location_t locus = LOCATION_LOCUS (location);
  return locus != UNKNOWN_LOCATION && locus != fun->function_end_locus;
}
